...
def generateRandomString(length: int) -> str:
    ...
    # Pick the first character completely at random
    result = [random.choice(CHARACTERS)]
    
    # Pick subsequent characters from the remaining choices (excluding the last picked one)
    for _ in range(length - 1):
        allowed = [c for c in CHARACTERS if c != result[-1]]
        result.append(random.choice(allowed))
...
def main():
    ...
    for i in range(instances):
        ...
        # 1. Generate random word string over {a, b, c} (guaranteed no adjacent duplicates)
