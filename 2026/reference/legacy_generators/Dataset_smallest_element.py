...
def descent_string_to_smallest_id(descent_str: str) -> int:
    """
    Finds the smallest generator present in the descent string based on order
    ('a' < 'b' < 'c') and maps it to its categorical integer token ID:
    'a' -> 1, 'b' -> 2, 'c' -> 3. An empty descent set returns 0.
    """
    if not descent_str:
        return 0
    ...
    # Filter out characters that aren't 'a', 'b', or 'c' if any exist,
    # then sort using the order() function values to find the absolute minimum
    valid_chars = [ch for ch in descent_str if ch in mapping]
    if not valid_chars:
        return 0
        
    smallest_char = min(valid_chars, key=order)
    return mapping[smallest_char]
...
def main():
    ...
    #   descents -> list-string of per-prefix right-descent token IDs of the smallest descent
    ...
    for i in range(instances):
        ...
        # 3. Fetch step-by-step per-prefix descent sets, then extract the smallest element ID
        step_descents = GetStepDescents(word)
        smallest_ids = [descent_string_to_smallest_id(d) for d in step_descents]

        word_cols.append([str(x) for x in token_ids])
        desc_cols.append([str(x) for x in smallest_ids])
