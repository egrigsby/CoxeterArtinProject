...
def load_descent_dataset(csv_path, n_generators):
    ...
    build_descent_dataset.py and returns three aligned tensors:
      ...
      mask    : [N, seq_len]              1 for valid descent positions, 0 for ignored prefixes/padding

    Column `descents` stores one bitmask int per position (bit j <=> generator j+1),
    with -1 at padding and uncalculated prefix positions; we decode it into the multi-hot target.
    """
    ...
    bitmasks = torch.tensor(descs, dtype=torch.long)      # [N, seq_len], -1 on padding/ignored prefixes

    # FIX: Only train/evaluate on positions where a descent was actually computed (not -1)
    mask = (bitmasks != -1).long()                        # [N, seq_len]
    ...
    targets = targets * mask.unsqueeze(-1).float()        # zero out padding and ignored prefix rows
