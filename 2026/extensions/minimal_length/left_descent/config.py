"""
Minimal length - per-prefix LEFT descent sets over ALL reduced words of A2~ up to
length 16 (5,259 words, 80/20). Dataset: ../build_minimal_length_datasets.py.
"""
...
# `descents` (per-prefix RIGHT-descent bitmask, padded with -1). Words are ALL
# minimal-length (reduced) words of length 1..16 (A2~, 5259 words), built by
# ../build_minimal_length_datasets.py and split 80/20 (train on 4207 words,
# test on 1052, shuffle seed 0). The Left test uses the identical words and
# partition — only the labels differ.
TRAIN_CSV      = "train.csv"
TEST_CSV       = "test.csv"
...
SEQUENCE_LENGTH     = 16        # max word length (= FIXED_LENGTH in ../build_minimal_length_datasets.py); must match the number of columns per row in TRAIN_CSV/TEST_CSV
LAYERS              = 1
HEADS               = 4
DIM_HEADS           = 64
