"""
Build the RIGHT-descent counterpart of the "Affine A3 Len18" experiment.

Reads ../"Affine A3 Len18"/train.csv and test.csv (exhaustive ShortLex
normal-form words of length 1..18 in A3~, 4254 words, split 30/70) and rewrites
ONLY the labels: the `descents` column becomes the per-prefix RIGHT descent set
(bitmask int, bit j <=> generator j+1, '-1' at padding). The `word` column and
the train/test partition are copied unchanged, so the two experiments differ in
nothing but the descent direction.

Self-contained: embeds the geometric (Tits) representation machinery verbatim
from the source generator (right_descent_path is the original from
"../../Data Generation/descents.py"; left_descent_path is kept for the
...
COXETER_MATRIX = [[1, 3, 2, 3],
                  [3, 1, 3, 2],
                  [2, 3, 1, 3],
                  [3, 2, 3, 1]]     # A3~
SOURCE_DIR = "left"                # experiment whose words/partition are inherited
OUT_DIR    = "right"               # this builder writes the RIGHT-descent run
TRAIN_CSV  = "train.csv"
TEST_CSV   = "test.csv"
...
def main():
    ...
    for name in (TRAIN_CSV, TEST_CSV):
        ...
        out.to_csv(f"{OUT_DIR}/{name}", index=False)
        print(f"Wrote {len(out)} rows to {name} (words and split inherited from "
              f"{SOURCE_DIR}/{name} -> {OUT_DIR}/{name}, labels = per-prefix RIGHT descents)")
