"""
Affine A3~ (4 generators) - per-prefix RIGHT descent sets. Same words and partition as
left/. Dataset: ../build_right_descent_nf_dataset.py.
"""
...
# `descents` (per-prefix RIGHT-descent bitmask, padded with -1). Words are the
# exhaustive set of ShortLex normal-form words of length 1..18 (A3~, 4254 words),
# with words and 30/70 split inherited unchanged from "../Affine A3 Len18" —
# only the labels differ (right instead of left descents). Built by
# build_right_descent_nf_dataset.py in this folder (the job script runs it
# before training).
TRAIN_CSV      = "train.csv"
TEST_CSV       = "test.csv"
...
SEQUENCE_LENGTH     = 18        # max word length (= FIXED_LENGTH in build_left_descent_nf_dataset.py); must match the number of columns per row in TRAIN_CSV/TEST_CSV
LAYERS              = 1
HEADS               = 4
DIM_HEADS           = 64
DIM_MODEL           = 256       # should equal HEADS * DIM_HEADS
DIM_MLP             = 1024       # hidden dim of MLP block; typically 4 * DIM_MODEL
TOKEN_TYPES         = 5         # input vocab size: #generators + 1 padding token (A2~: 3 gens + pad = 4)
DIM_OUTPUT          = 4         # multi-label head: one independent sigmoid unit per generator (A2~: 3)
