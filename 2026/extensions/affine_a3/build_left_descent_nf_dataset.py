...
This is exhaustive: every element of length 1..MAX_LEN appears exactly once
(A3~: 4254 words at MAX_LEN = 18). The ShortLex language is prefix-closed —
every prefix of a dataset word is itself a ShortLex normal form.
...
COXETER_MATRIX = [[1, 3, 2, 3],
                  ...
                  [2, 3, 1, 3],
                  [3, 2, 3, 1,]]      # A3~
MAX_LEN      = 18                 # enumerate ALL normal-form words of length 1..MAX_LEN
FIXED_LENGTH = 18                 # pad to this; must equal SEQUENCE_LENGTH in config.py
SEED         = 0                  # shuffle seed for the train/test split
TRAIN_FRAC   = 0.3
OUT_DIR      = "left"        # this builder writes the LEFT-descent run
TRAIN_CSV    = "left/train.csv"
TEST_CSV     = "left/test.csv"
...
def run_smoke_tests(words, matrix, mats, crosscheck_max_len=8):
    ...
    for l in range(1, MAX_LEN + 1):
        assert counts[l] == 4 * l, f"shell {l}: got {counts[l]}, expected {4 * l}"
