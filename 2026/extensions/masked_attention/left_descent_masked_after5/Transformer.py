...
def pad_mask_hook(attn_scores, hook, mask, num_masked_prefix=5):
    """
    attn_scores: [batch, head, q_pos, k_pos]
    mask: [batch, seq_len]  (1 for real tokens, 0 for padding)
    num_masked_prefix: The number of prefix tokens at the start to block out (indices 0..4).
    """
    # Create position mask: 0 for prefix tokens (indices 0..num_masked_prefix-1), 1 for downstream tokens
    position_mask = torch.ones_like(mask)
    position_mask[:, :num_masked_prefix] = 0  # <--- MARKS FIRST 5 TOKENS AS 0 (MASKED OUT)

    # Combine with padding mask: keep key ONLY if it is non-padding AND downstream of index 4
    combined_mask = mask * position_mask           # [batch, seq_len]

    pad_mask = combined_mask.unsqueeze(1).unsqueeze(2)  # [batch, 1, 1, seq_len]
    attn_scores = attn_scores.masked_fill(~pad_mask.bool(), float('-inf'))
    
    # Softmax safety check: prevent NaN errors for queries that are forced to attend only to downstream tokens
    seq_len = attn_scores.size(-1)
    eye = torch.eye(seq_len, device=attn_scores.device, dtype=torch.bool).unsqueeze(0).unsqueeze(0)
    q_disabled = (~combined_mask.bool()).unsqueeze(1).unsqueeze(-1)
...
def register_pad_mask_hook(model, attention_mask, num_masked_prefix=5):
    def mask_hook(attn_scores, hook):
        return pad_mask_hook(attn_scores, hook, attention_mask, num_masked_prefix=num_masked_prefix)
