...
def mask_first_5_tokens_mask(data_tensor, num_masked_prefix=5):
    """
    1 = Keep (attend / evaluate)
    0 = Mask out (first 5 tokens or padding)
    """
    ...
    # 2. Mask indices < 5 (0, 1, 2, 3, 4)
    pos_indices = torch.arange(seq_len, device=data_tensor.device).unsqueeze(0)
    after_first_5_mask = pos_indices >= num_masked_prefix
    
    # Combine: Must be non-padding AND position >= 5
    combined_mask = real_tokens_mask & after_first_5_mask
