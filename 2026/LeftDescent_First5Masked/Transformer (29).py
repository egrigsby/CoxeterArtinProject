...
def mask_after_pos_5_mask(data_tensor, cutoff_index=5):
    """
    1 = Keep (attend / evaluate)
    0 = Mask out (indices > cutoff_index or padding)
    """
    ...
    # 2. Keep indices <= 5 (indices 0, 1, 2, 3, 4, 5) and mask indices > 5
    pos_indices = torch.arange(seq_len, device=data_tensor.device).unsqueeze(0)
    up_to_pos_5_mask = pos_indices <= cutoff_index
    
    # Combine: Must be non-padding AND position <= 5
    combined_mask = real_tokens_mask & up_to_pos_5_mask
...
def pad_mask_hook(attn_scores, hook, mask):
    ...
    # 2. To avoid NaN in softmax for disabled query positions (> 5),
    ...
    # Force diagonal to 0.0 where query is disabled AND key == query position
...
def load_train_test_data(device1):
    ...
    # Generate mask that keeps indices 0..5 and masks out > 5 + padding
    train_att_mask = mask_after_pos_5_mask(train_tokens, cutoff_index=5)
    test_att_mask  = mask_after_pos_5_mask(test_tokens,  cutoff_index=5)

    return {
        "train_tokens": train_tokens,   "test_tokens": test_tokens,
        "train_targets": train_targets, "test_targets": test_targets,
        "train_mask": train_att_mask,   "test_mask": test_att_mask,
        "train_attention_mask": train_att_mask, "test_attention_mask": test_att_mask,
    }
