...
def load_class_dataset(csv_path):
    ...
    aligned tensors:
      ...
      targets : [N, seq_len]  integer class id per position (-1 at padding)
      mask    : [N, seq_len]  1 for real letters, 0 for padding

    Column `labels` stores one class id per position (task-specific meaning —
    see the build script in this folder), with -1 at padding positions.
    """
    ...
    targets = torch.tensor(labels, dtype=torch.long)      # [N, seq_len], -1 on padding
