"""
Build the RIGHT-ASCENT counterpart of the "Affine A2 Right" experiment.

Reads ../right_descent/train.csv and test.csv (exhaustive ShortLex
normal-form words of length 1..36 in A2~, split 30/70, words and partition
identical to "Affine A2") and rewrites ONLY the labels: the `descents` column
becomes the per-prefix RIGHT ASCENT set (bitmask int, bit j <=> generator j+1,
'-1' at padding). The `word` column and the train/test partition are copied
unchanged, so the two experiments differ in nothing but descent-vs-ascent
labels.

In any Coxeter group l(ws) = l(w) +- 1, so every generator is exactly one of
{right ascent, right descent} of an element: the ascent bitmask is
...
../right_descent/build_right_descent_dataset.py) and cross-checked against
...
SOURCE_DIR = "../right_descent"   # experiment whose words/partition are inherited
TRAIN_CSV  = "train.csv"
TEST_CSV   = "test.csv"
...
def right_descent_path(word, coxeter_matrix, mats=None, tol=1e-9):
    ...
    negative root, i.e. the j-th column of the accumulated reflection-matrix
    product has every coord <= 0 and is nonzero.
    """
    if mats is None:
...
# ---------------------------------------------------------------------------
# Relabel: right descents -> right ascents (complement), cross-checked per position
# ---------------------------------------------------------------------------
...
def relabel(df, matrix, mats):
    ...
    for w_str, d_str in zip(df["word"], df["descents"]):
        ...
        path = right_descent_path(word, matrix, mats=mats)  # one descent set per prefix
...
def main():
    ...
    for name in (TRAIN_CSV, TEST_CSV):
        ...
        print(f"Wrote {len(out)} rows to {name} (words and split inherited from "
              f"{SOURCE_DIR}/{name}, labels = per-prefix RIGHT ascents)")
