"""
Normal-form arm - per-prefix RIGHT ascent sets (complement of the right descent sets).
Same words and partition as right_descent/. Dataset: build_right_ascent_dataset.py.
"""
...
# `descents` (column name kept for the shared loader; VALUES are the per-prefix
# RIGHT-ASCENT bitmask = complement of the right-descent bitmask, padded with -1).
# Words and the 30/70 partition (train on 599 words, test on 1399) are inherited
# unchanged from "../Affine A2 Right" (exhaustive ShortLex normal-form words of length
# 1..36, A2~); only the labels differ — rebuilt by build_right_ascent_dataset.py here.
TRAIN_CSV      = "train.csv"
TEST_CSV       = "test.csv"
...
SEQUENCE_LENGTH     = 36        # max word length (= FIXED_LENGTH in the inherited datasets); must match the number of columns per row in TRAIN_CSV/TEST_CSV
LAYERS              = 1
HEADS               = 4
DIM_HEADS           = 64
